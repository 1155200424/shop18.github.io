  ...
  <div id="myList" class="myList">
    <ul>
      <li v-for="item in list" :key="item.product_id">
        ...
        <router-link :to="{ path: '/goods/details', query: {productID:item.id,seckill:seckill} }">
          <img :src="require(`../assets${item.imgUrl}`)" alt />
          <h2>{{item.goodsName}}</h2>
          <h3>{{item.goodsAddress}}</h3>
          <p>
            <span v-if="seckill">秒杀价 {{item.goodsDiscount}}元</span>
            <span v-else>{{item.goodsDiscount}}元</span>
            <span
              v-show="item.goodsPrice != item.goodsDiscount"
              class="del"
            >{{item.goodsPrice}}元</span>
          </p>
        </router-link>
      </li>
...
export default {
  ...
  // list为父组件传过来的商品列表
  // isMore为是否显示“浏览更多”
  props: ["list", "isMore", "isDelete","seckill"],
  data() {
    return {};
  },
  computed: {
